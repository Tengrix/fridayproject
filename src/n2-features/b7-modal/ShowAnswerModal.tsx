...
// }, [dispatch])
  ...
  const body = (
    ...
    <div style={modalStyle} className={classes.paper}>
      ...
      <span>Question number: {countA}/5 {countA===5 && <Button color="secondary" variant={"outlined"} onClick={()=>{startAgain()}}>start again</Button>}</span>
      <div>
                {questions[0+numQA]}
                
      </div>
      <div>
                <Button color="primary" variant={"outlined"}  disabled={false} onClick={showAnswer}>show answer</Button >
            </div>
            <div>
      ...
      {countA===5?<Button color="primary" variant={"outlined"} disabled={true} onClick={() => {
                    startAgain()
                }}> Next </Button>:<Button color="primary" variant={"outlined"} disabled={false} onClick={() => {
                    nextQuestion()
                }}> Next </Button>}
    </div>
  );
